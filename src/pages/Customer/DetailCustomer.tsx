...
export default function DetailCustomer() {
  ...
  const items: DescriptionsProps["items"] = [
    ...
    },
    {
      ...
      children:
        formatMoney(
          customerData?.transaction.reduce(
  ...
  const transactionsByDate: {
    [key: string]: { totalAmount: number; date: Date };
  } = {};
  ...
  const groupedTransactions: GroupedTransaction[] = Object.entries(
    transactionsByDate
  ).map(([code, { totalAmount, date }]) => ({
    date,
    totalAmount,
    code,
  }));
  ...
  const columns: ColumnType<GroupedTransaction>[] = [
    ...
    },
    {
      ...
      render: (date: Date) => (
        <p>
          <strong className="w-12 inline-block">Ngày:</strong>{" "}
          ...
          <br />
          <strong className="w-12 inline-block">Giờ:</strong>{" "}
  ...
  return (
    <div className="px-4">
      ...
      <Title level={5}>
        Tổng số lượng giao dịch: {groupedTransactions.length}
      </Title>
